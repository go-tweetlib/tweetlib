...
// Service: unifies all the available Twitter API service
// such as Timelines, Help, etc.
type Service struct {
	client *http.Client
...
// TimelinesListCall returns a collection of the most recent Tweets posted by
// the user indicated by the screenname or user id parameters.
//
// e.g.
//    client.Timelines.List("twitteruser").Do()
...
// Count will add the 'count' paramenter to the request which will
// limit the maximum number of tweets returned
//
// e.g.
//   client.Timelines.List("twitteruser").Count(5).Do()
...
// authenticating user and the users they follow. Up to 800 Tweets are
// obtainable on the home timeline.
//
// e.g
//    client.Timelines.HomeTimeline().Do()
...
// Count limits the number of tweets returned to a maximum of count
//
// e.g.
//    client.Timelines.HomeTimeline().Count(10).Do()
func (c *TimelinesHomeTimelineCall) Count(count int) *TimelinesHomeTimelineCall {
	c.opt_["count"] = count
	return c
}

// SinceId allows you to tell twitter to return only tweets with an Id
// higher than sinceId.
//
// e.g.
//    client.Timelines.HomeTimeline().SinceId("12345678").Do()
func (c *TimelinesHomeTimelineCall) SinceId(since_id string) *TimelinesHomeTimelineCall {
	c.opt_["since_id"] = since_id
	return c
}

// MaxId will limit the tweets return to tweets with an Id lower or equal to
// max_id
//
// e.g.
//    client.Timelines.HomeTimeline().MaxId("12345678").Do()
